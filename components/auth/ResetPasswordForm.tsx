...
export default function ResetPasswordForm() {
  ...
  // SECURITY GUARD: Tendang user kalau iseng akses halaman ini langsung tanpa email
  useEffect(() => {
    if (!targetEmail) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!targetEmail) return;
    if (token.length !== 6) return toast.error("Token harus 6 digit angka.");
    ...
    >
      <div className="hidden md:flex w-1/2 bg-slate-900 p-12 flex-col items-center justify-center relative overflow-hidden order-2 md:order-1">
        ...
        <p className="text-slate-400 text-sm text-center mt-4 max-w-xs relative z-10 leading-relaxed">
          Masukkan token 6-digit yang telah dikirim ke{" "}
        ...
        <form onSubmit={onSubmit} className="space-y-5">
          <motion.div variants={itemVariants} className="space-y-2">
            ...
            <input
              ...
              maxLength={6}
              value={token}
              onChange={(e) => setToken(e.target.value.replace(/[^0-9]/g, ""))} // Hanya angka
              placeholder="000000"
              required
